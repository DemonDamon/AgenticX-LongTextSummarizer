...
export default function MultiDoc() {
  ...
  const [result, setResult] = useState<{
    ...
    trace: { llmCalls: number; promptTokens: number; intent: string; docCount: number; model?: string; durationMs?: number };
  } | null>(null);
  ...
  const mutation = trpc.summarize.collection.useMutation({
    onSuccess: (data) => {
      setResult(data);
      if (isAuthenticated) {
        utils.history.listCollection.invalidate();
      }
    },
  });

  const { data: collectionHistory, isLoading: histLoading } = trpc.history.listCollection.useQuery(undefined, {
        ...
        <div className="container flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="ml-2 border-l border-border/40 pl-2">
              <ModelKeyPanel model={model} apiKey={apiKey} onModelChange={setModel} onApiKeyChange={setApiKey} />
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-3 space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={loadSamples} className="flex-1 py-2 text-sm border border-border rounded-lg text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors font-mono">
                加载示例
              </button>
              <button onClick={addDoc} className="flex-1 py-2 text-sm border border-border rounded-lg text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors">
                + 添加文档
              </button>
            </div>
              ...
              {docs.map((doc, idx) => (
                <div key={doc.docId} className="blueprint-card rounded-xl border border-border/60 overflow-hidden">
                  <div className="px-4 py-2.5 bg-muted/30 border-b border-border/40 flex items-center gap-2">
                    ...
                    <button onClick={() => removeDoc(doc.docId)} className="text-muted-foreground/50 hover:text-red-500 transition-colors ml-1">
              ...
              <>
                ...
                <div className="blueprint-card rounded-xl border border-border/60 overflow-hidden">
                  ...
                  <div className="p-4 grid grid-cols-5 gap-3">
                    ...
                    <div className="text-center border-r border-border/40">
                      <div className="text-lg font-black font-mono text-green-600">
                        {formatDuration(result.trace.durationMs)}
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">耗时</div>
                    </div>
                    <div className="text-center">
                      <div className={`font-mono font-bold text-sm px-2 py-1 rounded ${INTENT_COLORS[result.trace.intent] ?? "bg-muted text-foreground"}`}>
                        {result.trace.intent}
                      </div>
                  ...
                  {result.trace.model && (
                    <div className="px-4 pb-3">
                      <span className="mono-label text-[10px] text-muted-foreground">model: </span>
                      <span className="mono-label text-[10px] text-primary">{result.trace.model}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="blueprint-card rounded-xl border border-primary/30 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded font-mono ${INTENT_COLORS[result.trace.intent] ?? "bg-muted text-foreground"}`}>
                      {result.trace.intent}
                    </span>
                  </div>
          ...
          <div className="xl:col-span-3">
            <div className="blueprint-card rounded-xl border border-border/60 overflow-hidden sticky top-20">
              <div className="px-4 py-2.5 bg-muted/50 border-b border-border/40 flex items-center justify-between">
                <span className="mono-label text-[11px] text-muted-foreground">// collection_history</span>
              ...
              {!isAuthenticated && (
                <div className="p-4 text-center">
                  <div className="text-xs text-muted-foreground mb-1">登录后查看历史</div>
                  <div className="mono-label text-[10px] text-muted-foreground/40">// auth_required</div>
                </div>
              )}

              {isAuthenticated && histLoading && (
                <div className="p-4 flex justify-center">
                  <div className="w-5 h-5 rounded-full border-2 border-primary/20 border-t-primary animate-spin" />
                </div>
              )}

              {isAuthenticated && !histLoading && (!collectionHistory || collectionHistory.length === 0) && (
                <div className="p-4 text-center">
                  <div className="text-xs text-muted-foreground">暂无多文档历史</div>
                  <div className="mono-label text-[10px] text-muted-foreground/40 mt-1">// records: []</div>
                </div>
              )}
                ...
                <div className="divide-y divide-border/30 max-h-[600px] overflow-y-auto">
                  {collectionHistory.map((item) => (
                    <div key={item.id} className="text-xs">
                      <button
                        onClick={() => setExpandedHistId(expandedHistId === item.id ? null : item.id)}
                        className="w-full px-3 py-2.5 text-left hover:bg-muted/30 transition-colors"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-1.5 py-0.5 rounded font-mono text-[10px] font-bold ${INTENT_COLORS[item.intent] ?? "bg-muted text-foreground"}`}>
                            {item.intent}
                          </span>
                          <span className="text-muted-foreground text-[10px]">{item.docCount} 篇</span>
                          <svg
                            className={`w-3 h-3 text-muted-foreground ml-auto transition-transform ${expandedHistId === item.id ? "rotate-180" : ""}`}
                            fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
                          >
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                          </svg>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-muted-foreground/70 text-[10px]">{formatDate(item.createdAt)}</span>
                          <span className="text-primary/70 text-[10px] font-mono">{item.llmCalls}calls</span>
                          <span className="text-primary/70 text-[10px] font-mono">{item.promptTokens}tk</span>
                          <span className="text-cyan-600 text-[10px] font-mono">{formatDuration(item.durationMs)}</span>
                        </div>
                      </button>

                      {expandedHistId === item.id && (
                        <div className="px-3 pb-3 bg-muted/10">
                          {/* 4-metric cards */}
                          <div className="grid grid-cols-2 gap-1.5 mb-2 p-2 bg-muted/30 rounded-lg">
                            <div className="text-center py-1">
                              <div className="font-black font-mono text-primary text-sm">{item.llmCalls}</div>
                              <div className="text-[9px] text-muted-foreground">调用次数</div>
                            </div>
                            <div className="text-center py-1 border-l border-border/40">
                              <div className="font-black font-mono text-primary text-sm">{item.promptTokens}</div>
                              <div className="text-[9px] text-muted-foreground">Token 消耗</div>
                            </div>
                            <div className="text-center py-1 border-t border-border/40">
                              <div className="font-black font-mono text-cyan-600 text-sm">{formatDuration(item.durationMs)}</div>
                              <div className="text-[9px] text-muted-foreground">耗时</div>
                            </div>
                            <div className="text-center py-1 border-t border-l border-border/40">
                              <div className="font-mono text-foreground text-[10px] font-semibold truncate px-1">{item.stages}</div>
                              <div className="text-[9px] text-muted-foreground">处理阶段</div>
                            </div>
                          </div>
                          {item.model && (
                          ...
                          <div className="mono-label text-[10px] text-muted-foreground mb-1">// synthesis</div>
                          <div className="text-[11px] text-foreground leading-relaxed line-clamp-5">
                            {item.synthesisText}
                          </div>
                        </div>
                      )}
